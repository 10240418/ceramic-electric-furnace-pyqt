...
class CardStatusDb30(QFrame):
    ...
    def create_device_card(self, device: dict, index: int):
        ...
        # D (Done)
        done_badge = self.create_value_badge("D", "1" if device.get("done") else "0", device.get("done"), has_error)
        layout.addWidget(done_badge)
        
        layout.addSpacing(4)
        
        # B (Busy)
        busy_badge = self.create_value_badge("B", "1" if device.get("busy") else "0", device.get("busy"), has_error)
        layout.addWidget(busy_badge)

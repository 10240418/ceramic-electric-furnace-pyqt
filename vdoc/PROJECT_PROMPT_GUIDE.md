...
# 初始化服务管理器
service_manager = ServiceManager(use_mock=True)
...
**功能**:
- 使用 PyQt6 信号机制，将后端数据安全传递到主线程
- 支持弧流数据、传感器数据、批次状态、错误信息、连接状态
...
data_bridge.error_occurred.connect(self.on_error)
```
...
```python
# 弧流数据
cache.set_arc_data(data)           # 存储弧流数据
cache.get_arc_data()                # 获取最新弧流数据
cache.get_arc_history(count=100)    # 获取弧流历史数据

# 传感器数据
cache.set_sensor_data(data)         # 存储传感器数据
cache.get_sensor_data()             # 获取最新传感器数据
cache.get_sensor_history(count=100) # 获取传感器历史数据

# 批次状态
cache.set_batch_status(status)      # 存储批次状态
cache.get_batch_status()            # 获取批次状态

# 实时显示数据（高频调用）
cache.get_realtime_display_data()   # 获取前端实时显示数据
```

### 4. PLCManager - PLC 长连接管理器

**位置**: `backend/plc/plc_manager.py`

**功能**:
- 维护 PLC 长连接（避免频繁连接/断开）
- 自动重连机制
- 连接健康检查
- 线程安全读写
...
# 连接 PLC
success, msg = plc_manager.connect()

# 读取 DB 块
data = plc_manager.read_db(db_number=32, offset=0, size=29)

# 写入 DB 块
success, msg = plc_manager.write_db(db_number=1, offset=0, data=data)

# 断开连接
plc_manager.disconnect()
```

### 5. BatchService - 批次状态管理

**位置**: `backend/services/batch_service.py`

**功能**:
- 维护全局冶炼状态 (is_smelting)
- 维护当前批次编号 (batch_code)
- 支持暂停/恢复冶炼（保留批次号）
- 断电恢复保护（状态持久化到文件）
...
# 开始冶炼
result = batch_service.start(batch_code="26010315")

# 暂停冶炼
result = batch_service.pause()

# 恢复冶炼
result = batch_service.resume()
...
state = batch_service.state
```

### 6. PollingLoops - 轮询服务

**位置**: `backend/services/polling_loops_v2.py`
...
| 轮询任务 | 间隔 | 批量写入 | 写入条件 |
|---------|------|---------|---------|
| DB1 弧流弧压 | 0.2s(冶炼中)/5s(空闲) | 20次→4秒 | 有批次号且状态=running/paused |
| DB32 传感器 | 0.5s | 30次→15秒 | 有批次号且状态=running/paused |
| 料仓重量 | 0.5s | 30次→15秒 | 有批次号且状态=running/paused |
| DB30/DB41 状态 | 5s | 不写库，仅缓存 | - |
...
# 切换到高速模式（冶炼中）
switch_db1_speed(high_speed=True)  # 0.2s

# 切换到低速模式（空闲）
switch_db1_speed(high_speed=False) # 5s
```
...
# 检查缓存中是否有数据
data = self.data_cache.get_arc_data()
print(data)
```
...
```powershell
# 正确
cd ceramic-electric-furnace-pyqt; python main.py
...
不要创建多余的 md/py/test 文件，测试完毕后一定要删除，并且测试代码不要使用 emoji
...
目录务必整洁，修改代码时删除旧代码，不要冗余

### 6. 回答执行规范

你是一个很严格的 python pyqt6 写上位机的高手，你很严谨认真，且对代码很严苛，不会写无用冗余代码，并且很多问题，对于我希望实现的效果和架构你会认真思考，如果我的提议不好或者你有更好的方案，你会规劝我。

### 7. 最高优先级

对于我的最主要的一个功能就是我的这个 pyqt 的窗口和我的 py 的后端的服务的话，不能死机卡死，这是最主要也是优先级最高的一个要求。

### 8. 反驳我的回答

对于我说的需求等的话，肯定会有一些东西说的不专业，如果你理解了的话，就回答我，"大王，小的罪该万死，但是这个XXXX"这样回答。

### 9. 编码问题

我的代码文件肯定会就是有中文和 python 代码，以及可能会有图标，所以的话，生成的代码需要规避编码问题错误。

### 10. log 以及代码文件

我的代码文件以及 log 的输出的话，等一切不要使用图标等标注。

### 11. 不要虚构

回答我以及生成的 md 文件之中一定要和我的实际的代码文件相关，而不是虚构的。
...
- `vdoc/CORE_COMPONENTS_GUIDE.md` - 核心 UI 组件使用指南
- `vdoc/FRONTEND_DATA_FLOW.md` - 前端数据流转完整说明
- `vdoc/BUSINESS_LOGIC_DB32_DETAIL.md` - DB32 MODBUS 数据详解
- `README.md` - 项目说明

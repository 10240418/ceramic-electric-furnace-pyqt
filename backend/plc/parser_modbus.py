...
class ModbusDataParser:
    ...
    def __init__(self, 
        ...
        """
        self.config_path = Path(config_path) if config_path else self.PROJECT_ROOT / "backend" / "configs" / "config_L3_P2_F2_C4_db32.yaml"
        self.module_path = Path(module_path) if module_path else self.PROJECT_ROOT / "backend" / "configs" / "plc_modules.yaml"
    ...
    def parse_flow(self, data: bytes, offset: int, scale: float = 0.01) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f" 解析流量失败 @ offset {offset}: {e}")
    ...
    def parse_valve_control(self, data: bytes, offset: int) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f" 解析蝶阀状态失败 @ offset {offset}: {e}")
